...
void *os_realloc(void *ptr, size_t size)
{
	...
	if (!ptr) {
		...
		if (block) {
			...
			if (block->status == STATUS_FREE && size > block->size && !block->next) {
				struct block_meta *new;

				new = sbrk((block_size - block->size) - 56);
				return block + 1;
			}
		}

		block = os_malloc(block_size);
		return block;
	}
	...
	if (block->status == STATUS_ALLOC) {
		if (block->next) {
			if (block->next->status == STATUS_FREE) {
				block->size += block->next->size + ALIGN(META_SIZE) + ALIGN(1);
		...
		new = os_malloc(size);
		memmove(new, block + 1, block->size + ALIGN(META_SIZE));
		os_free(ptr);
